...
export function PublishModal({ open, onClose }: PublishModalProps) {
  ...
  const handleConfirmPublish = () => {
    const res = campusStore.publish();
    onClose();
    toast({
      type: "success",
      title: "Digital Twin Published!",
      description: `Version ${res.version} is now live for all users.`,
    });
  };
        ...
        >
          ...
          <div className="space-y-2 text-sm text-[rgb(var(--muted-fg))] leading-relaxed">
            ...
            <p className="text-xs italic text-[rgb(var(--muted-fg))]/80">
              All saved draft changes, nodes, edges, buildings, and navigation paths will become active immediately.
            </p>
          </div>
